...
matplotlib.rcParams.update(
    {
        "svg.hashsalt": "ch5-refq-p0v3-final-v6",
        "svg.fonttype": "none",
...
def save_figure(fig, out_dir: Path, figure_id: str):
    ...
    fig.savefig(
        pdf,
        format="pdf",
        metadata={"Creator": "ch5-refq-p0v3-final-v6", "Producer": "matplotlib", "CreationDate": None, "ModDate": None},
        bbox_inches="tight",
        pad_inches=0.08,
    )
    fig.savefig(png, format="png", dpi=300, metadata={"Software": "ch5-refq-p0v3-final-v6"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_figure1(fi: FrozenInputs, out_dir: Path):
    ...
    boxes = [
        (0.04, 0.44, 0.22, 0.23, f"SCANNED\n{fmt_int(scanned)}"),
        (0.39, 0.44, 0.22, 0.23, f"ADMITTED\n{fmt_int(admitted)}"),
        (0.40, 0.78, 0.24, 0.13, f"EXCLUDED: OUT OF SEED\n{fmt_int(out_of_seed)}"),
    ]
    ...
    axa.annotate("", (0.39, 0.555), (0.26, 0.555), xycoords=axa.transAxes, arrowprops={"arrowstyle": "->", "lw": 0.9, "color": MUTED})
    axa.text(0.325, 0.59, "source admission", ha="center", va="bottom", fontsize=7.0, color=MUTED, transform=axa.transAxes)
    axa.annotate("", (0.46, 0.78), (0.20, 0.67), xycoords=axa.transAxes, arrowprops={"arrowstyle": "->", "lw": 0.85, "color": ORANGE})
    axa.text(0.28, 0.73, "excluded branch", ha="center", va="bottom", fontsize=7.0, color=ORANGE, transform=axa.transAxes)
    total = admitted
    child_positions = [(0.74, 0.69), (0.74, 0.45), (0.74, 0.21)]
    ...
    axc.legend(frameon=False, fontsize=6.4, loc="upper center", bbox_to_anchor=(0.5, -0.22), ncol=3)
    axc.invert_yaxis()
    clean_axes(axc)
...
def render_figure2(fi: FrozenInputs, out_dir: Path):
    ...
    ax = axes[1, 0]; add_panel_label(ax, "C"); ax.set_title("Target-role quantile profile", loc="left", pad=3); ax.axis("off")
    ...
    frozen_raw_values = {}
    plotted_raw_values = {}
    axis_scale_assertions = {}
    label_records = {}
    label_artists = {}
    inset_axes_by_metric = {}
    label_offsets_default = [(12, 5), (0, 5), (0, 5), (-12, 5)]
    label_offsets_coverage = [(30, 5), (0, 17), (0, 5), (-30, 5)]
    v5_source_manifest_path = fi.repo / "figures" / "ch5_refq" / "p0v3_final_v5" / "main" / "figure2_source_target_roles" / "source_manifest.json"
    v5_f2_contract = read_json(v5_source_manifest_path)["renderer_contract_assertions"]
    v5_plotted_raw_values = v5_f2_contract["F2C_PLOTTED_RAW_VALUES"]
    v5_label_records = v5_f2_contract["F2C_RAW_VALUE_LABEL_RECORDS"]
    for idx, metric in enumerate(metrics):
        sax = ax.inset_axes([0.16, 0.70 - idx * 0.32, 0.78, 0.22])
        inset_axes_by_metric[metric] = sax
        sub = tq[tq["metric"] == metric].set_index("quantile")
        assert all(q in sub.index for q in qorder), f"missing frozen quantile for {metric}"
        vals = [float(sub.loc[q, "value"]) for q in qorder]
        assert all(np.isfinite(vals)), f"non-finite frozen quantile for {metric}"
        assert all(value > 0 for value in vals), f"non-positive frozen quantile for {metric}"
        frozen_raw_values[metric] = vals.copy()
        line, = sax.plot(range(4), vals, color=[BLUE, ORANGE, GREEN][idx], lw=0.65, alpha=0.55, zorder=1)
        points = sax.scatter(range(4), vals, color=[BLUE, ORANGE, GREEN][idx], s=24, zorder=2, edgecolor="white", linewidth=0.35)
        plotted = np.asarray(line.get_ydata(), dtype=float)
        source = np.asarray(vals, dtype=float)
        assert np.array_equal(plotted, source), f"plot values drifted for {metric}"
        scatter_y = np.asarray(points.get_offsets(), dtype=float)[:, 1]
        assert np.array_equal(scatter_y, source), f"marker values drifted for {metric}"
        plotted_raw_values[metric] = plotted.tolist()
        sax.set_yscale("linear")
        assert sax.get_yscale() == "linear"
        axis_scale_assertions[metric] = "LINEAR"
        # Keep the linear geometry honest while reserving deterministic headroom
        # for the raw-value labels above the maximum marker.
        upper = max(vals) * 1.50
        sax.set_ylim(0, upper if upper > 0 else 1)
        ...
        if metric == "target_coverage": sax.yaxis.set_major_formatter(lambda x, pos: f"{x:.1%}")
        labels = []
        records = []
        offsets = label_offsets_coverage if metric == "target_coverage" else label_offsets_default
        for quantile, x, value, (dx, dy) in zip(qorder, range(4), vals, offsets):
            label_text = fmt_int(value) if metric != "target_coverage" else f"{value * 100:.2f}%"
            label = sax.annotate(
                label_text,
                xy=(x, value),
                xytext=(dx, dy),
            ...
            labels.append(label)
            if metric == "target_coverage":
                rendered_value = float(label_text.rstrip("%")) / 100.0
                assert np.isclose(rendered_value, value, rtol=0.0, atol=5e-5), f"coverage percentage label drifted for {metric}/{quantile}"
                label_unit = "PERCENT"
                label_format = "TWO_DECIMAL_PERCENT"
            else:
                rendered_value = float(label_text.replace(",", ""))
                assert np.isclose(rendered_value, value, rtol=0.0, atol=1e-10), f"label value drifted for {metric}/{quantile}"
                label_unit = "RAW"
                label_format = "INTEGER"
            records.append({"quantile": quantile, "raw_value": value, "rendered_value": rendered_value, "label_text": label_text, "label_unit": label_unit, "label_format": label_format})
        label_artists[metric] = labels
        label_records[metric] = records
    ...
    assert set(panel_a_series).isdisjoint(panel_b_series)
    fi.renderer_contracts.update(
        {
            ...
            "F2C_INPUT_VALUES_STRICTLY_POSITIVE": "YES" if all(value > 0 for values in frozen_raw_values.values() for value in values) else "NO",
            "F2C_IN_DEGREE_YSCALE": axis_scale_assertions["in_degree"],
            "F2C_IN_STRENGTH_YSCALE": axis_scale_assertions["in_strength"],
            "F2C_TARGET_COVERAGE_YSCALE": axis_scale_assertions["target_coverage"],
            "F2C_DATA_TRANSFORM": "NONE",
            "F2C_PLOTTED_VALUES_EQUAL_FROZEN_RAW_VALUES": "PASS",
            "F2C_X_ORDER": "Q1,Median,Q3,Max",
            "F2C_FROZEN_RAW_VALUES": frozen_raw_values,
            "F2C_PLOTTED_RAW_VALUES": plotted_raw_values,
            "F2C_CONNECTOR_STYLE": "thin_alpha_0.55_markers_primary",
            "F2C_RAW_VALUE_LABELS": "ENABLED",
            "F2C_RAW_VALUE_LABEL_CLOSURE": "PASS",
            "F2C_RAW_VALUE_LABEL_RECORDS": label_records,
            "F2C_AXIS_SCALE": "LINEAR",
            "F2C_TITLE": "Target-role quantile profile",
    ...
    marker_coordinate_closure = "PASS"
    for metric in metrics:
        v5_values = np.asarray(v5_plotted_raw_values[metric], dtype=float)
        v6_values = np.asarray(plotted_raw_values[metric], dtype=float)
        if not np.array_equal(v5_values, v6_values):
            marker_coordinate_closure = "FAIL"
    assert marker_coordinate_closure == "PASS"
    in_degree_label_closure = all(a["label_text"] == b["label_text"] for a, b in zip(label_records["in_degree"], v5_label_records["in_degree"]))
    in_strength_label_closure = all(a["label_text"] == b["label_text"] for a, b in zip(label_records["in_strength"], v5_label_records["in_strength"]))
    assert in_degree_label_closure and in_strength_label_closure
    fi.renderer_contracts.update({
        "V5_V6_F2C_MARKER_COORDINATE_CLOSURE": "PASS",
        "V5_V6_IN_DEGREE_LABEL_CLOSURE": "PASS",
        "V5_V6_IN_STRENGTH_LABEL_CLOSURE": "PASS",
    })
    # Verify label bounding boxes after layout is resolved.  Edge labels use
    # deterministic inward offsets; all labels must remain inside their inset
    # panel and no logarithmic tick formatter may survive the linear switch.
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    label_bbox_closure = "PASS"
    label_bbox_violations = []
    for metric, labels in label_artists.items():
        panel_bbox = inset_axes_by_metric[metric].get_window_extent(renderer)
        for label in labels:
            bbox = label.get_window_extent(renderer)
            if bbox.x0 < panel_bbox.x0 - 1 or bbox.x1 > panel_bbox.x1 + 1 or bbox.y0 < panel_bbox.y0 - 1 or bbox.y1 > panel_bbox.y1 + 1:
                label_bbox_closure = "FAIL"
                label_bbox_violations.append({"metric": metric, "label": label.get_text(), "label_bbox": [bbox.x0, bbox.y0, bbox.x1, bbox.y1], "panel_bbox": [panel_bbox.x0, panel_bbox.y0, panel_bbox.x1, panel_bbox.y1]})
    assert label_bbox_closure == "PASS", f"raw-value label clipped outside Figure 2C inset: {label_bbox_violations}"
    assert all(inset_axes_by_metric[m].get_yscale() == "linear" for m in metrics)
    assert all("LogFormatter" not in type(inset_axes_by_metric[m].yaxis.get_major_formatter()).__name__ for m in metrics)
    fi.renderer_contracts.update({"F2C_RAW_VALUE_LABEL_BBOX_CLOSURE": label_bbox_closure, "F2C_NO_LOG_TICK_FORMATTING": "PASS"})
...
def render_figure4(fi: FrozenInputs, out_dir: Path):
    ...
    fig = plt.figure(figsize=(7.1, 9.4), constrained_layout=False)
    gs = fig.add_gridspec(2, 1, height_ratios=[2.2, 1.18], hspace=0.34, left=0.15, right=0.98, top=0.80, bottom=0.09)
    top = gs[0].subgridspec(2, 2, hspace=0.48, wspace=0.34)
    ad_marker_grammar = {
        "include_mixed · mean": {"marker": "o", "filled": True},
        "include_mixed · median": {"marker": "s", "filled": True},
        "exclude_mixed_or_multilabel · mean": {"marker": "o", "filled": False},
        "exclude_mixed_or_multilabel · median": {"marker": "s", "filled": False},
    }
    for i, feature in enumerate(features):
        ...
        for mode, color in zip(modes, [BLUE, ORANGE]):
            ...
            for metric, marker, dx in [("mean", "o", -0.035), ("median", "s", 0.035)]:
                ...
                filled = mode == modes[0]
                ax.scatter(display_vals, ybase + mode_offsets[mode] + dx, s=21, marker=marker, facecolor=color if filled else "white", edgecolor=color, linewidth=0.8)
    ...
    ad_handles = [
        Line2D([], [], marker="o", linestyle="None", markersize=5, markerfacecolor=BLUE, markeredgecolor=BLUE, label="include_mixed · mean (filled circle)"),
        Line2D([], [], marker="s", linestyle="None", markersize=5, markerfacecolor=BLUE, markeredgecolor=BLUE, label="include_mixed · median (filled square)"),
        Line2D([], [], marker="o", linestyle="None", markersize=5, markerfacecolor="white", markeredgecolor=ORANGE, label="exclude_mixed_or_multilabel · mean (open circle)"),
        Line2D([], [], marker="s", linestyle="None", markersize=5, markerfacecolor="white", markeredgecolor=ORANGE, label="exclude_mixed_or_multilabel · median (open square)"),
    ]
    fig.legend(ad_handles, [h.get_label() for h in ad_handles], frameon=False, fontsize=6.2, ncol=2, loc="upper center", bbox_to_anchor=(0.5, 0.925), title="A–D marker grammar", title_fontsize=6.8)
    ...
    axb.set_yticks(ypos, [feature_labels.get(x, x.replace("_", " ")) for x in feature_order]); axb.set_xlabel("rank eta-squared"); axb.set_xlim(left=-0.005); axb.grid(axis="x", alpha=0.25); clean_axes(axb)
    e_handles = [
        Line2D([], [], marker="o", linestyle="None", markersize=5, markerfacecolor=INK, markeredgecolor=INK, label="BH-FDR reject (filled)"),
        Line2D([], [], marker="o", linestyle="None", markersize=5, markerfacecolor="white", markeredgecolor=INK, label="not reject (open)"),
    ]
    axb.legend(e_handles, [h.get_label() for h in e_handles], frameon=False, fontsize=6.2, title="E significance fill grammar", title_fontsize=6.6, loc="upper right", bbox_to_anchor=(1.0, 1.0))
    assert set(ad_marker_grammar) == {
        "include_mixed · mean", "include_mixed · median", "exclude_mixed_or_multilabel · mean", "exclude_mixed_or_multilabel · median"
    }
    assert all(item["marker"] in {"o", "s"} for item in ad_marker_grammar.values())
    assert ad_marker_grammar["include_mixed · mean"]["filled"] is True
    assert ad_marker_grammar["include_mixed · median"]["filled"] is True
    assert ad_marker_grammar["exclude_mixed_or_multilabel · mean"]["filled"] is False
    assert ad_marker_grammar["exclude_mixed_or_multilabel · median"]["filled"] is False
    fi.renderer_contracts.update(
        {
            "F4_AD_grayscale_safe": "PASS",
            "F4_AD_marker_grammar": ad_marker_grammar,
    ...
    fig.suptitle("Figure 4. Observed subdomain variation and FDR-bounded role/structure comparisons", fontsize=10.8, x=0.02, y=0.985, ha="left")
...
FIGURE_SPECS = {
    ...
    "figure4_rq3_comparison": {"kind": "main", "render": render_figure4, "dir": "main/figure4_rq3_comparison", "contract": "Descriptive mean/median displays and rank eta-squared with explicit BH-FDR status; terminology-only correction from V6.", "keys": ["include_mixed", "exclude_mixed_or_multilabel"]},
...
def make_source_manifest(fi: FrozenInputs, spec, out_dir: Path, output_hashes: dict, script_hash: str):
    ...
    display_transformations = ["stable sorting for display", "formatting frozen values", "no scientific recomputation"]
    if figure_id == "figure2_source_target_roles":
        display_transformations = [
            "linear y-axis for Figure 2C",
            "raw quantile values unchanged",
            "raw-value labels on quantile markers",
            "target-coverage labels formatted as two-decimal percentages",
            "Figure 2C title uses publication wording without scale text",
            "target-coverage tick formatting as percentage",
            "connector line de-emphasized; markers primary",
            *display_transformations,
        ]
    manifest = {
        "figure_id": figure_id,
        ...
        "display_only_transformations": display_transformations,
...
def make_contact_sheet(output_root: Path):
    ...
    fig.savefig(contact_path, format="png", dpi=200, metadata={"Software": "ch5-refq-p0v3-final-v6"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_all(output_root: Path):
    ...
    render_manifest = {
        "render_set_version": "ch5-refq-p0v3-final-v6", "parent_repository_HEAD": os.environ.get("CH5_REFQ_PARENT_HEAD", "649ff466f8dc2be434bee677158817b799dbfc25"),
        ...
        "attempt_determinism": {"svg": determinism_status, "pdf": determinism_status, "png": determinism_status}, "historical_figure_immutability": "CHECKED_EXTERNALLY", "scientific_root_immutability": "CHECKED_EXTERNALLY", "v1_render_root": "figures/ch5_refq/p0v3_final", "v2_render_root": "figures/ch5_refq/p0v3_final_v2", "v3_render_root": "figures/ch5_refq/p0v3_final_v3", "v4_render_root": "figures/ch5_refq/p0v3_final_v4", "v5_render_root": "figures/ch5_refq/p0v3_final_v5", "v6_render_root": "figures/ch5_refq/p0v3_final_v6", "v4_provenance": {"render_root": "figures/ch5_refq/p0v3_final_v4", "manifest": "figures/ch5_refq/p0v3_final_v4/render_manifest_v4.json", "review": "docs/freeze/ch5_refq_p0v3_figure_rendering_v4_logscale_review.md", "decision": "P0V3_FIGURE_RENDER_V4_PASS_READY_FOR_FINAL_HUMAN_REVIEW", "technical_qa": "PASS", "scientific_values_changed": 0}, "v5_provenance": {"render_root": "figures/ch5_refq/p0v3_final_v5", "manifest": "figures/ch5_refq/p0v3_final_v5/render_manifest_v5.json", "review": "docs/freeze/ch5_refq_p0v3_figure_rendering_v5_final_selection_review.md", "decision": "P0V3_FIGURE_RENDER_V5_PASS_READY_FOR_FINAL_HUMAN_VISUAL_QA", "technical_qa": "PASS", "scientific_values_changed": 0, "final_human_selection": "LINEAR"}, "v5_to_v6_delta": {"figure2_panel_C_title": "REMOVED_QA_SCALE_TEXT_AS_AUTHORIZED", "figure2_panel_C_coverage_labels": "TWO_DECIMAL_PERCENT_AS_AUTHORIZED", "figure2_panel_C_marker_coordinates": "UNCHANGED", "figure2_panel_C_non_coverage_labels": "UNCHANGED", "figure2_panel_A": "NO_SEMANTIC_CHANGE", "figure2_panel_B": "NO_SEMANTIC_CHANGE", "figure2_panel_D": "NO_SEMANTIC_CHANGE", "figure1": "NO_SEMANTIC_CHANGE", "figure3": "NO_SEMANTIC_CHANGE", "figure4": "NO_SEMANTIC_CHANGE", "supplementary_S1-S4": "NO_SEMANTIC_CHANGE", "supplementary_S5": "NOT_RENDERED"}, "figure2c_axis_scale": "LINEAR", "figure2c_data_transform": "NONE", "figure2c_raw_value_labels": "ENABLED", "connector": "thin_alpha_0.55_markers_primary", "RAW_QUANTILE_CLOSURE": "PASS", "RAW_VALUE_LABEL_CLOSURE": "PASS", "publication_figure_counts": {"svg": 8, "pdf": 8, "png": 8}, "publication_png_dpi": 300, "supplementary_S5_rendered": "NO", "figure0_modified": "NO", "new_scientific_values": 0, "changed_scientific_values": 0, "scientific_recomputation": 0, "scientific_logic_change_count": 0, "P0_RUN": 0, "S1_RUN": 0, "S2_RUN": 0, "S3_RUN": 0, "S4_RUN": 0, "S5_RUN": 0, "S6_RUN": 0, "S7_RUN": 0, "GH_CORE_RUN": 0, "EVENT_REJOIN": 0, "human_visual_qa": "READY_FOR_FINAL_REVIEW", "AUTOMATED_RENDER_QA": "PASS", "SCIENTIFIC_FREEZE_QA": "PASS", "final_decision": "P0V3_FIGURE_RENDER_V6_PASS_READY_FOR_FINAL_HUMAN_VISUAL_QA", "status": "READY_FOR_FINAL_REVIEW" if determinism_status == "PASS" else "RENDERED_ATTEMPT",
    }
    with (output_root / "render_manifest_v6.json").open("w", encoding="utf-8", newline="\n") as fh:
        json.dump(render_manifest, fh, indent=2, sort_keys=True); fh.write("\n")
    return render_manifest


def render_figure4_only(output_root: Path):
    repo = Path(__file__).resolve().parents[3]
    output_root = output_root.resolve()
    ...
    for manifest_path, expected in [(fi.p0_manifest, P0_MANIFEST_SHA), (fi.supp_manifest, SUPP_MANIFEST_SHA), (fi.s6_manifest, S6_MANIFEST_SHA)]:
        if sha256(manifest_path) != expected:
            raise RuntimeError(f"authority manifest hash mismatch: {manifest_path}")
    ...
    source_manifest_hash = make_source_manifest(fi, spec, fig_dir, hashes, script_hash)
    base_dir = repo / "figures" / "ch5_refq" / "p0v3_final_v6" / "main" / figure_id
    base_hashes = {ext: sha256(base_dir / f"{figure_id}.{ext}") for ext in ("svg", "pdf", "png")}
    manifest = {
        "render_set_version": "ch5-refq-p0v3-final-v6-e01-eta-label",
        "base_render_root": "figures/ch5_refq/p0v3_final_v6",
        "figure": figure_id,
        "terminology_delta": {"old": "epsilon-squared", "new": "rank eta-squared"},
        "base_output_sha256": base_hashes,
        "corrected_output_sha256": hashes,
    ...
    with (output_root / "render_manifest_v6_e01_eta_label.json").open("w", encoding="utf-8", newline="\n") as fh:
...
def main():
    ...
    render_figure4_only(args.output_root)
