...
matplotlib.rcParams.update(
    {
        "svg.hashsalt": "ch5-refq-p0v3-final-v1",
        "svg.fonttype": "none",
...
def save_figure(fig, out_dir: Path, figure_id: str):
    ...
    fig.savefig(
        pdf,
        format="pdf",
        metadata={"Creator": "ch5-refq-p0v3-final-v1", "Producer": "matplotlib", "CreationDate": None, "ModDate": None},
        bbox_inches="tight",
        pad_inches=0.08,
    )
    fig.savefig(png, format="png", dpi=300, metadata={"Software": "ch5-refq-p0v3-final-v1"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_figure1(fi: FrozenInputs, out_dir: Path):
    ...
    assert flow_map["reference_records_before_source_admission"] - flow_map["out_of_seed_source_observation_reference_records"] == flow_map["admitted_source_observation_reference_records"]
    assert sum(flow_map[k] for k in ["target_project_mappable_records", "target_non_project_records", "target_unresolved_records"]) == flow_map["admitted_source_observation_reference_records"]
    ...
    boxes = [
        (0.02, 0.47, 0.19, 0.25, f"Scanned\n{fmt_int(flow_map['reference_records_before_source_admission'])}"),
        (0.28, 0.47, 0.19, 0.25, f"Out of seed\n{fmt_int(flow_map['out_of_seed_source_observation_reference_records'])}"),
        (0.54, 0.47, 0.19, 0.25, f"Admitted\n{fmt_int(flow_map['admitted_source_observation_reference_records'])}"),
    ]
    for x, y, w, h, txt in boxes:
        axa.add_patch(patches.FancyBboxPatch((x, y), w, h, boxstyle="round,pad=0.015", facecolor="#EAF2F8", edgecolor=BLUE, linewidth=0.8, transform=axa.transAxes))
        axa.text(x + w / 2, y + h / 2, txt, ha="center", va="center", fontsize=9, transform=axa.transAxes)
    for x in [0.21, 0.47]:
        axa.annotate("", (x + 0.05, 0.595), (x, 0.595), xycoords=axa.transAxes, arrowprops={"arrowstyle": "->", "lw": 0.8, "color": MUTED})
    status = [
    ...
    total = flow_map["admitted_source_observation_reference_records"]
    for i, (name, count, color) in enumerate(status):
        x, y = 0.78, 0.76 - i * 0.23
        axa.add_patch(patches.FancyBboxPatch((x, y), 0.20, 0.17, boxstyle="round,pad=0.012", facecolor=color, alpha=0.16, edgecolor=color, linewidth=0.7, transform=axa.transAxes))
        axa.text(x + 0.10, y + 0.085, f"{name.replace('_', ' ').title()}\n{fmt_int(count)} ({count / total:.2%})", ha="center", va="center", fontsize=7.7, transform=axa.transAxes)
    axa.annotate("", (0.78, 0.66), (0.73, 0.59), xycoords=axa.transAxes, arrowprops={"arrowstyle": "->", "lw": 0.8, "color": MUTED})
    axa.text(0.02, 0.10, "1,586,047 records are the quotient-eligible, project-mappable subset; units remain Reference records.", transform=axa.transAxes, fontsize=7.8, color=MUTED)
    ...
    order = sorted(membership["event_type"].unique())
...
def render_figure2(fi: FrozenInputs, out_dir: Path):
    ...
    for metric, color, label in [("out_degree", BLUE, "out-degree"), ("out_strength", ORANGE, "out-strength")]:
    ...
    for idx, metric in enumerate(metrics):
        ...
        sax.set_xticks(range(4), ["Q25", "Med", "Q75", "Max"]); sax.set_title(metric_labels[metric], fontsize=7.5, loc="left", pad=1); sax.grid(axis="y", alpha=0.25); clean_axes(sax)
...
def render_figure3(fi: FrozenInputs, out_dir: Path):
    ...
    runs = runs.sort_values("seed").reset_index(drop=True); canonical = int(stab["canonical_seed"]); run_index = np.arange(len(runs))
    c1.plot(run_index, runs["community_count"], color=BLUE, lw=1.0); c1.scatter([0], [int(runs.iloc[0]["community_count"])], color=ORANGE, zorder=3); c1.set_ylabel("count"); c1.set_xlabel("run index"); c1.set_title("community count", fontsize=8); clean_axes(c1)
    c2.plot(run_index, runs["modularity"], color=GREEN, lw=1.0); c2.scatter([0], [float(runs.iloc[0]["modularity"])], color=ORANGE, zorder=3); c2.set_ylabel("modularity"); c2.set_xlabel("run index"); c2.set_title("modularity", fontsize=8); clean_axes(c2)
    c3.plot(run_index, runs["ari_to_canonical"], color=PURPLE, lw=1.0); c3.axhline(0.9, color=MUTED, ls="--", lw=0.7); c3.scatter([0], [1.0], color=ORANGE, zorder=3); c3.set_ylabel("ARI"); c3.set_xlabel("run index"); c3.set_title("ARI to canonical", fontsize=8); clean_axes(c3)
...
def render_s3(fi: FrozenInputs, out_dir: Path):
    ...
    top = gs[0].subgridspec(1, 2, wspace=0.32)
    ax = fig.add_subplot(top[0]); add_panel_label(ax, "A"); ax.set_title("Spearman stability", loc="left", pad=3); ax.hist(1.0 - runs["spearman_to_canonical"], bins=10, color=BLUE, alpha=0.85); ax.axvline(1.0 - summary["spearman_summary"]["min"], color=MUTED, ls="--", lw=0.8); ax.set_xlabel("1 - Spearman to canonical"); ax.set_ylabel("runs"); clean_axes(ax)
...
def render_all(output_root: Path):
    ...
    render_manifest = {
        "render_set_version": "ch5-refq-p0v3-final-v1", "parent_repository_HEAD": os.environ.get("CH5_REFQ_PARENT_HEAD", "fbb6114fd83c1fab503dadce470d9fa9ccee59f6"),
