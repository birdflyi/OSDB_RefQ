...
matplotlib.rcParams.update(
    {
        "svg.hashsalt": "ch5-refq-p0v3-final-v5",
        "svg.fonttype": "none",
...
def save_figure(fig, out_dir: Path, figure_id: str):
    ...
    fig.savefig(
        pdf,
        format="pdf",
        metadata={"Creator": "ch5-refq-p0v3-final-v5", "Producer": "matplotlib", "CreationDate": None, "ModDate": None},
        bbox_inches="tight",
        pad_inches=0.08,
    )
    fig.savefig(png, format="png", dpi=300, metadata={"Software": "ch5-refq-p0v3-final-v5"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_figure2(fi: FrozenInputs, out_dir: Path):
    ...
    ax = axes[1, 0]; add_panel_label(ax, "C"); ax.set_title("Target-role quantile profile (linear y-scales)", loc="left", pad=3); ax.axis("off")
    ...
    for idx, metric in enumerate(metrics):
        ...
        for quantile, x, value, (dx, dy) in zip(qorder, range(4), vals, offsets):
            label_text = fmt_int(value) if metric != "target_coverage" else format(value, ".10g")
            label = sax.annotate(
                label_text,
                xy=(x, value),
                xytext=(dx, dy),
            ...
            records.append({"quantile": quantile, "raw_value": value, "rendered_value": rendered_value, "label_text": label_text})
...
def make_contact_sheet(output_root: Path):
    ...
    fig.savefig(contact_path, format="png", dpi=200, metadata={"Software": "ch5-refq-p0v3-final-v5"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_all(output_root: Path):
    ...
    render_manifest = {
        "render_set_version": "ch5-refq-p0v3-final-v5", "parent_repository_HEAD": os.environ.get("CH5_REFQ_PARENT_HEAD", "6087b3b45de5d1a0496238a6c18bdd51ee4002cf"),
