...
matplotlib.rcParams.update(
    {
        "svg.hashsalt": "ch5-refq-p0v3-final-v4",
        "svg.fonttype": "none",
...
def save_figure(fig, out_dir: Path, figure_id: str):
    ...
    fig.savefig(
        pdf,
        format="pdf",
        metadata={"Creator": "ch5-refq-p0v3-final-v4", "Producer": "matplotlib", "CreationDate": None, "ModDate": None},
        bbox_inches="tight",
        pad_inches=0.08,
    )
    fig.savefig(png, format="png", dpi=300, metadata={"Software": "ch5-refq-p0v3-final-v4"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_figure2(fi: FrozenInputs, out_dir: Path):
    ...
    ax = axes[1, 0]; add_panel_label(ax, "C"); ax.set_title("Target-role quantile profile (log y-scales)", loc="left", pad=3); ax.axis("off")
    ...
    for idx, metric in enumerate(metrics):
        ...
        sax.set_yscale("log", base=10)
        assert sax.get_yscale() == "log"
        assert getattr(sax.yaxis._scale, "base", None) == 10
        axis_scale_assertions[metric] = "log10"
...
def make_source_manifest(fi: FrozenInputs, spec, out_dir: Path, output_hashes: dict, script_hash: str):
    ...
    if figure_id == "figure2_source_target_roles":
        display_transformations = [
            "base-10 logarithmic y-axis for Figure 2C",
...
def make_contact_sheet(output_root: Path):
    ...
    fig.savefig(contact_path, format="png", dpi=200, metadata={"Software": "ch5-refq-p0v3-final-v4"}, bbox_inches="tight", pad_inches=0.08)
    plt.close(fig)
...
def render_all(output_root: Path):
    ...
    render_manifest = {
        "render_set_version": "ch5-refq-p0v3-final-v4", "parent_repository_HEAD": os.environ.get("CH5_REFQ_PARENT_HEAD", "984b46c5f007c1c15cb4df51641d98ba05841532"),
